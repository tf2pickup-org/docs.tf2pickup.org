...
```json
{
  ...
  "_embedded": {
    "events": [
      ...
      { "type": "gameEnded", "at": "2024-01-01T01:00:00.000Z", "reason": "match ended" }
    ]
  }
}
```
...
| `gameServerAssigned`         | `gameServerName`                               | A game server was assigned              |
| `gameServerInitialized`      |                                                | Game server finished configuration      |
| `gameServerConfigureFailed`  |                                                | Game server configuration failed        |
| `gameStarted`                |                                                | Game became live                        |
| `gameRestarted`              |                                                | Game server was restarted               |
| `roundEnded`                 | `winner`, `lengthMs`, `score`                  | A round finished                        |
| `substituteRequested`        | `gameClass`                                    | A substitute was requested              |
| `playerReplaced`             | `gameClass`                                    | A player was substituted                |
| `playerJoinedGameServer`     | `player` (steamId)                             | A player connected to the game server   |
| `playerJoinedGameServerTeam` | `player` (steamId), `team`                     | A player joined their assigned team     |
| `playerLeftGameServer`       | `player` (steamId)                             | A player disconnected from game server  |
| `gameEnded`                  | `reason`                                       | Game ended                              |

`gameEnded.reason` is one of `"match ended"`, `"interrupted"`, or `"too many substitute requests"`.
...
- `config.classes[].canMakeFriendsWith` — optional; present only for classes that support the friend system.
